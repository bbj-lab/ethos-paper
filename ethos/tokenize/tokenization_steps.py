...
def load_and_process_data(
    ...
    module = import_module(
        f".{data_prop.module}.preprocessors", package="ethos.tokenize"
    )
    registered_data = [
        cls
        for name, cls in module.__dict__.items()
        if isinstance(cls, type)
        and issubclass(cls, TimeData)
        and cls != TimeData
    ]
...
def merge_timeline_chunks(
    patient_timeline_chunks,
...
def normalize_mimic_times(
    patient_timelines: dict, data_prop: DataProp, **kwargs
):
...
def _inject_separators(
    patient_id, times, event_tokens, injector: SeparatorInjector
):
...
def concatenate_timelines(
    ...
    patient_indices = np.cumsum(
        [0] + [len(patient_times) for patient_times in times[:-1]]
    )
...
def estimate_true_sep_time(times, tokens, vocab):
    ...
    sep_estimated_times = (
        times[separator_indices + 1] - times[separator_indices - 1]
    )
    ...
    sep_df = sep_df.groupby("separator").agg(
...
def count_clock_tokens(tokens, vocab) -> dict[str, int]:
    clock_lookup = {c: vocab.stoi[c] for c in SpecialToken.CLOCK_NAMES}
    clocks = sorted(clock_lookup.values())
    cts = np.histogram(tokens, bins=clocks + [max(clocks) + 1])[0]
    return dict(zip(SpecialToken.CLOCK_NAMES, cts))
...
def get_context_data(data_prop, vocab, **kwargs):
    ...
    context_data = [
        ...
        for name, cls in module.__dict__.items()
        if isinstance(cls, type)
        and issubclass(cls, ContextData)
        and cls != ContextData
    ]
    age_reference = None
    ...
    patient_ids = set.intersection(
        *[set(chunk.keys()) for chunk in context_chunks]
    )
    context = {
        patient_id: np.concatenate(
            [chunk[patient_id] for chunk in context_chunks]
        )
...
def dump_timelines(
    ...
    with h5py.File(timelines_path, "w") as f:
        ...
        f.create_dataset(
            "patient_data_offsets", data=list(patient_idx_to_id.keys())
        )
        patient_context = np.asarray(
            [context_data[patient_id] for patient_id in patient_ids],
            dtype=TOKEN_DTYPE,
        )
        ...
        age_reference = np.fromiter(
            (age_reference[patient_id] for patient_id in patient_ids),
            dtype=np.int32,
        )
